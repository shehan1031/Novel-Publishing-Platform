...
const Badge = ({ type, label }) => {
  const m = {
    active: "#22c55e", published: "#22c55e", completed: "#22c55e",
    banned: "#ef4444", failed: "#ef4444", cancelled: "#ef4444",
...
/* ── Pure SVG sparkline — no external deps, no CDN ── */
const Sparkline = ({ data = [], color = "#3b82f6", fill = true, height = 180, label = "" }) => {
  ...
  const pts = vals.map((v, i) => ({
    x: PAD.left + i * xStep,
    y: PAD.top  + iH - ((v - minV) / (maxV - minV)) * iH,
  }));
  ...
  const yTicks = [0, 1, 2, 3].map(i => {
    ...
    const y = PAD.top + iH - ((v - minV) / (maxV - minV)) * iH;
    return { v, y };
  });
  ...
  data.forEach((d, i) => {
    if (i % step === 0 || i === data.length - 1) {
      ...
      xLabels.push({
        x: PAD.left + i * xStep,
        label: `${dt.getDate()} ${dt.toLocaleString("default", { month: "short" })}`,
      });
    }
  });
...
const BarChart = ({ data = [], color = "#8b5cf6", height = 180 }) => {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: H, display: "block" }}>
      ...
      {yTicks.map((t, i) => (
        <text key={i} x={PAD.left - 5} y={t.y + 4} textAnchor="end"
          fill="#475569" fontSize="9" fontFamily="system-ui">
          {Math.round(t.v)}
        </text>
      ))}
        ...
        <div className="v6-content">
            ...
            <>
              ...
              <div className="v6-kpi-grid">
                {[
                  {
                    label: "Total Revenue",
                    val:   fmtLKR(stats?.revenueAll),
                    sub:   `${Number(stats?.revenueGrowth) >= 0 ? "+" : ""}${stats?.revenueGrowth ?? 0}% vs prev month`,
                    up:    Number(stats?.revenueGrowth) >= 0,
                    color: "#3b82f6",
                  },
                  {
                    label: "Total Users",
                    val:   fmtNum(stats?.totalUsers),
                  ...
                  },
                  {
                    ...
                    val:   fmtNum(stats?.publishedNovels),
                    sub:   `${fmtNum(stats?.totalNovels)} total`,
                    up:    true,
                    color: "#14b8a6",
                  },
                  {
                    label: "Banned Users",
                    val:   fmtNum(stats?.bannedUsers),
                    sub:   `${fmtNum(stats?.bannedNovels)} novels banned`,
                    up:    false,
                    color: "#ef4444",
                  },
                ].map((k, i) => (
                  <div key={i} className="v6-kpi" style={{ "--kpi-accent": k.color, animationDelay: i * 0.07 + "s" }}>
                    ...
                    <div className={`v6-kpi-sub ${k.up ? "up" : "dn"}`}>
                      {k.up ? I.up : I.down} {k.sub}
                    </div>
              ...
              <div className="v6-row2">
                ...
                <div className="v6-card">
                  ...
                  {(stats?.topNovels || []).length === 0
                    ? <Empty msg="No novels yet." />
          ...
          {section === "analytics" && (
            <>
              ...
              <div className="v6-row2" style={{ gridTemplateColumns: "1fr 1fr" }}>
                <div className="v6-card">
                  <div className="v6-card-head">
                    <span className="v6-card-title">Revenue / day (30 days)</span>
                  </div>
                  <div style={{ padding: "12px 16px 8px" }}>
                    <Sparkline data={revDays} color="#3b82f6" label="revenue" height={180} />
                  </div>
                ...
                <div className="v6-card">
                  <div className="v6-card-head">
                    <span className="v6-card-title">New users / day (30 days)</span>
                  </div>
                  <div style={{ padding: "12px 16px 8px" }}>
                    <BarChart data={usrDays} color="#8b5cf6" height={180} />
                  </div>
                </div>
              </div>
              ...
              <div className="v6-card">
                <div className="v6-card-head">
                  <span className="v6-card-title">Revenue by package</span>
                </div>
            ...
            <div className="v6-card">
              <div className="v6-card-head">
                <span className="v6-card-title">
                  All users <span className="v6-count-chip">{users.length}</span>
                </span>
                <div className="v6-filter-row">
                  {["all", "reader", "author", "admin"].map(f => (
                    <button key={f} className="v6-filter-btn"
                      onClick={() => f === "all"
                        ? loadUsers("")
                        : API.get(`/admin/users?role=${f}&limit=100`).then(r => setUsers(r.data.users || []))
                      }>
                      {f}
                    </button>
                  ))}
              ...
              {loading ? <Loader /> : users.length === 0 ? <Empty /> : (
                <div className="v6-table-wrap">
                  <table className="v6-table">
                    <thead><tr>
                      <th>User</th><th>Role</th><th>Balance</th>
                          ...
                          <td>
                            <div className="v6-act-row">
                              <button className={`v6-act ${u.banned ? "success" : "warn"}`}
                                onClick={() => banUser(u._id, u.banned)}
                                title={u.banned ? "Unban" : "Ban"}>
                                {u.banned ? I.check : I.ban}
                              </button>
                              <button className="v6-act danger" onClick={() => deleteUser(u._id)}>
                                {I.trash}
                              </button>
                            </div>
                          </td>
          ...
          {section === "novels" && (
            <div className="v6-card">
              <div className="v6-card-head">
                <span className="v6-card-title">
                  All novels <span className="v6-count-chip">{novels.length}</span>
                </span>
                <div className="v6-filter-row">
                  {["all", "published", "draft", "banned"].map(f => (
                    <button key={f} className="v6-filter-btn"
                      onClick={() => f === "all"
                        ? loadNovels("")
                        : API.get(`/admin/novels?status=${f}&limit=100`).then(r => setNovels(r.data.novels || []))
                      }>
                      {f}
                    </button>
                  ))}
              ...
              {loading ? <Loader /> : novels.length === 0 ? <Empty /> : (
                <div className="v6-table-wrap">
                  <table className="v6-table">
                    <thead><tr>
                      <th>Novel</th><th>Author</th><th>Status</th>
                      <th>Chapters</th><th>Views</th><th>Created</th><th>Actions</th>
                    </tr></thead>
                    <tbody>
                      {novels.map(n => (
                        <tr key={n._id}>
                          <td>
                            <div className="v6-cell-name">{n.title}</div>
                            <div className="v6-cell-sub">{n.genre || "—"}</div>
                          </td>
                          <td className="v6-cell-sub">{n.author?.name || n.author?.email || "—"}</td>
                          <td>
                            <select className="v6-select"
                              value={n.status}
                              onChange={e => updateNovelStatus(n._id, e.target.value)}>
                          ...
                          <td>
                            <button className="v6-act danger" onClick={() => deleteNovel(n._id)}>
                              {I.trash}
                            </button>
                          </td>
                        </tr>
          ...
          {section === "chapters" && (
            <div className="v6-card">
              <div className="v6-card-head">
                <span className="v6-card-title">
                  All chapters <span className="v6-count-chip">{chapters.length}</span>
                </span>
                <div className="v6-filter-row">
                  {["all", "published", "draft", "banned"].map(f => (
                    <button key={f} className="v6-filter-btn"
                      onClick={() => f === "all"
                        ? loadChapters("")
                        : API.get(`/admin/chapters?status=${f}&limit=100`).then(r => setChapters(r.data.chapters || []))
                      }>
                      {f}
                    </button>
                  ))}
              ...
              {loading ? <Loader /> : chapters.length === 0 ? <Empty msg="No chapters found." /> : (
                <div className="v6-table-wrap">
                  <table className="v6-table">
                    <thead><tr>
                      <th>Chapter</th><th>Novel</th><th>Author</th><th>Status</th>
                      <th>Premium</th><th>Views</th><th>Created</th><th>Actions</th>
                    </tr></thead>
                    <tbody>
                      {chapters.map(c => (
                        <tr key={c._id}>
                          <td>
                            <div className="v6-cell-name">{c.title}</div>
                            <div className="v6-cell-sub">Order #{c.order}</div>
                          </td>
                          <td className="v6-cell-sub">{c.novel?.title || "—"}</td>
                          <td className="v6-cell-sub">{c.novel?.author?.name || c.novel?.author?.email || "—"}</td>
                          <td>
                            <select className="v6-select"
                              value={c.status || (c.banned ? "banned" : "published")}
                              onChange={e => updateChapterStatus(c._id, e.target.value)}>
                              <option value="draft">Draft</option>
                              <option value="published">Published</option>
                              <option value="banned">Banned</option>
                            </select>
                          ...
                          <td>
                            {c.isPremium
                              ? <Badge type="amber" label={`${c.coinCost} coins`} />
                              : <Badge type="active" label="Free" />
                            }
                          </td>
                          ...
                          <td>
                            <div className="v6-act-row">
                              <button className="v6-act warn"
                                ...
                                title={(c.banned || c.status === "banned") ? "Unban chapter" : "Ban chapter"}>
                                {(c.banned || c.status === "banned") ? I.check : I.ban}
                              </button>
                              <button className="v6-act danger" onClick={() => deleteChapter(c._id)}>
                                {I.trash}
                              </button>
                            </div>
                          </td>
          ...
          {section === "comments" && (
            <div className="v6-card">
              <div className="v6-card-head">
                <span className="v6-card-title">
                  All comments <span className="v6-count-chip">{comments.length}</span>
                </span>
              </div>
              {loading ? <Loader />
                : comments.length === 0
                  ? <Empty msg="No comments found. Make sure your Comment model exists and comments have been posted." />
                  : (
                    <div>
                      {comments.map((c, i) => (
                        <div key={c._id} className="v6-comment">
                          ...
                          <div className="v6-comment-body">
                            <div className="v6-comment-meta">
                              <span className="v6-comment-user">
                                {c.user?.name || c.user?.email || "Unknown user"}
                              </span>
                              {(c.novel || c.chapter) && (
                                <span className="v6-comment-on">
                                  on {c.novel?.title || c.chapter?.title || "—"}
                                </span>
                              )}
                              <span className="v6-comment-time">{fmtDate(c.createdAt)}</span>
                            </div>
                            <div className="v6-comment-text">
                              {c.content || c.text || "—"}
                            </div>
                          </div>
                          <button className="v6-act danger" onClick={() => deleteComment(c._id)}>
                            {I.trash}
                          </button>
                        </div>
                      ))}
          ...
          {section === "revenue" && (
            <>
              ...
              <div className="v6-card">
                <div className="v6-card-head">
                  <span className="v6-card-title">
                    All transactions <span className="v6-count-chip">{txns.length}</span>
                  </span>
                </div>
                {loading ? <Loader /> : txns.length === 0 ? <Empty msg="No transactions yet." /> : (
                  <div className="v6-table-wrap">
                    <table className="v6-table">
                      <thead><tr>
                        <th>User</th><th>Order ID</th><th>Package</th>
                        <th>Coins</th><th>Amount</th><th>Status</th><th>Date</th>
                      </tr></thead>
                      <tbody>
                        {txns.map(t => (
                          <tr key={t._id}>
                            <td>
                              <div className="v6-cell-name">{t.reader?.name || t.reader?.email || "—"}</div>
                            </td>
                            <td>
                              <div className="v6-cell-sub" style={{ fontFamily: "monospace", fontSize: 10 }}>
                                {(t.orderId || "—").slice(0, 24)}
                              </div>
                            </td>
                            <td className="v6-cell-sub">{t.packageId || "—"}</td>
                            <td style={{ fontWeight: 600, color: "#f59e0b", fontSize: 12 }}>
                              {fmtNum(t.totalCoins || t.coins || 0)}
                            </td>
                            <td style={{ fontWeight: 600, color: "#22c55e", fontSize: 13 }}>
                              {fmtLKR(t.amount)}
                            </td>
