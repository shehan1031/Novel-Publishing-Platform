...
exports.handleNotify = async (req, res) => {
  ...
  try {
    ...
    const baseCoins     = Number(txn.coins  || 0);
    const bonusCoins    = Number(txn.bonus  || 0);
    const totalToCredit = baseCoins + bonusCoins;
 
    // Read starting balance BEFORE incrementing for accurate ledger entries
    const targetUser = await User.findById(txn.reader);
    if (!targetUser) {
      console.error(`[PayHere] User not found: ${txn.reader}`);
      return res.status(404).send("User not found");
    }
    const startingBalance = Number(targetUser.balance || 0);
 
    // Credit all coins atomically
    ...
    // Sequential ledger entries with correct per-step balances
    const balanceAfterBase = startingBalance + baseCoins;
    await PointTransaction.create({
      ...
      description:  `Bought ${baseCoins} coins — LKR ${txn.amount}`,
    });
    ...
    if (bonusCoins > 0) {
      await PointTransaction.create({
        ...
        type:         "bonus",
        transaction:  txn._id,
        balanceAfter: balanceAfterBase + bonusCoins,
        description:  `Bonus ${bonusCoins} coins (${txn.packageId})`,
      });
    }
